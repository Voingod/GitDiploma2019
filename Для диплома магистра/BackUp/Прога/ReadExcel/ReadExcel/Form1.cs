...
namespace ReadExcel
{
    public partial class Form1 : Form
    {
        ...
        private void EuclidSquare(int k, out double min, out double nextMin, out List<double> SummClasters)
        {
            SummClasters = new List<double>();
            for (int i = 1; i < dataGridViewExcel.Columns.Count; ++i)
            {
                ...
                for (int j = 1; j < dataGridViewExcel.Rows.Count; ++j)
                {
                    if(dataGridViewExcelGlobal[j - 1, k - 1].Value.ToString().Length!=0)
                    {
                        dataExcelGlobalFormat = Convert.ToDouble(dataGridViewExcelGlobal[j - 1, k - 1].Value.ToString());
                    }
                    else
                    {
                        dataExcelGlobalFormat = Convert.ToDouble(dataGridViewExcelGlobal[j - 1, k - 1].Value=0);
                    }
                    double dataExcelFormat = Convert.ToDouble(dataGridViewExcel[i, j].Value.ToString());
        ...
        private void Culculate()
        {
            ...
            if (tbPathToFile.Visible == true)
            {
                try
                {
                    ...
                    {
                        work.CreateFile();
                        work.ReadFile(tbPathToFile, dataGridViewExcel, rbtnWoman, rbtnMan);
                    }
                    else
                    {
                    ...
                    for (int i = 0; i < SummClasters.Count; i++)
                    {
                        if (min == SummClasters[i])
                        {
                            switch (i)
                            {
                                ...
                                case 1:
                                    tbResult.Text += "The student belongs to the cluster *" + (i + 1) + "*" + Environment.NewLine +
                                "Мінімальна відстань до кластеру становить: " + (distantiont = Math.Round(SummClasters[i], 2)) + Environment.NewLine + Environment.NewLine +
                                "Кластер має наступні характристики: " + ReadFile(read, i + 1); cluster = i + 1; break;
                                case 2:
                                    tbResult.Text += "Студент відноситься до кластеру *" + (i + 1) + "*" + Environment.NewLine +
                        ...
                        for (int i = 0; i < SummClasters.Count; i++)
                        {
                            if (nextMin == SummClasters[i])
                            {
                                switch (i)
                                {
                                    ...
                                    "Кластер має наступні характристики: " + ReadFile(read, i + 1); break;
                                    case 2:
                                        tbRecomend.Text += "The next distance after the minimum is *" + Math.Round(SummClasters[i], 2) + "*"
        ...
        private string ReadFile(string read, int i)
        {
            ...
            string path = Environment.CurrentDirectory + "/Table/Character.xlsx";
        ...
        private void Global()
        {
            ...
            if (checkBoxGM.Checked)
            {
               // double dataExcelGlobalFormat;
                ...
                {
                    OleDbDataAdapter da = new OleDbDataAdapter("Select АТС0, АТД0, ЧСС0, АТС1, АТД1, ЧСС1, АТС2, АТД2, ЧСС2, АТС3, АТД3, ЧСС3, АТС4, АТД4, ЧСС4, АТС5, АТД5, ЧСС5 from [Ж$] ", conn);
                ...
                {
                    OleDbDataAdapter da = new OleDbDataAdapter("Select АТС0, АТД0, ЧСС0, АТС1, АТД1, ЧСС1, АТС2, АТД2, ЧСС2, АТС3, АТД3, ЧСС3, АТС4, АТД4, ЧСС4, АТС5, АТД5, ЧСС5 from [М$]", conn);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
                    dataGridViewExcelGlobal.AllowUserToAddRows = false;
                    dataGridViewExcelGlobal.DataSource = dt;
                ...
                for (int k = 1; k < dataGridViewExcelGlobal.Rows.Count+1; k++)
                {
                    double min = 0;
                    double nextMin = 0;
                    List<double> SummClasters;

                    EuclidSquare(k, out min, out nextMin, out SummClasters);
                    StreamWriter sw = new StreamWriter("D:\\minCluster.txt", true);
                    StreamWriter swDist = new StreamWriter("D:\\MinDist.txt", true);
                    for (int i = 0; i < SummClasters.Count; i++)
                    {
                        if (min == SummClasters[i])
                        {
                            switch (i)
                            {
                                case 0: sw.WriteLine(i + 1); sw.Close(); swDist.WriteLine(Math.Round(SummClasters[i], 2)); swDist.Close(); break;
                                case 1: sw.WriteLine(i + 1); sw.Close(); swDist.WriteLine(Math.Round(SummClasters[i], 2)); swDist.Close(); break;
                                case 2: sw.WriteLine(i + 1); sw.Close(); swDist.WriteLine(Math.Round(SummClasters[i], 2)); swDist.Close(); break;
                                case 3: sw.WriteLine(i + 1); sw.Close(); swDist.WriteLine(Math.Round(SummClasters[i], 2)); swDist.Close(); break;
                                case 4: sw.WriteLine(i + 1); sw.Close(); swDist.WriteLine(Math.Round(SummClasters[i], 2)); swDist.Close(); break;
                                case 5: sw.WriteLine(i + 1); sw.Close(); swDist.WriteLine(Math.Round(SummClasters[i], 2)); swDist.Close(); break;
                                case 6: sw.WriteLine(i + 1); sw.Close(); swDist.WriteLine(Math.Round(SummClasters[i], 2)); swDist.Close(); break;
                                case 7: sw.WriteLine(i + 1); sw.Close(); swDist.WriteLine(Math.Round(SummClasters[i], 2)); swDist.Close(); break;
                    ...
                    StreamWriter swNextDist = new StreamWriter("D:\\NextMinDist.txt", true);
                    for (int i = 0; i < SummClasters.Count; i++)
                    {
                        if (nextMin == SummClasters[i])
                        {
                            switch (i)
                            {
                                case 0: sw2.WriteLine(i + 1); sw2.Close(); swNextDist.WriteLine(Math.Round(SummClasters[i], 2)); swNextDist.Close(); break;
                                case 1: sw2.WriteLine(i + 1); sw2.Close(); swNextDist.WriteLine(Math.Round(SummClasters[i], 2)); swNextDist.Close(); break;
                                case 2: sw2.WriteLine(i + 1); sw2.Close(); swNextDist.WriteLine(Math.Round(SummClasters[i], 2)); swNextDist.Close(); break;
                                case 3: sw2.WriteLine(i + 1); sw2.Close(); swNextDist.WriteLine(Math.Round(SummClasters[i], 2)); swNextDist.Close(); break;
                                case 4: sw2.WriteLine(i + 1); sw2.Close(); swNextDist.WriteLine(Math.Round(SummClasters[i], 2)); swNextDist.Close(); break;
                                case 5: sw2.WriteLine(i + 1); sw2.Close(); swNextDist.WriteLine(Math.Round(SummClasters[i], 2)); swNextDist.Close(); break;
                                case 6: sw2.WriteLine(i + 1); sw2.Close(); swNextDist.WriteLine(Math.Round(SummClasters[i], 2)); swNextDist.Close(); break;
                                case 7: sw2.WriteLine(i + 1); sw2.Close(); swNextDist.WriteLine(Math.Round(SummClasters[i], 2)); swNextDist.Close(); break;
                            }
                        }
                    }
                }
