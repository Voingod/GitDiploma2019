...
namespace ReadExcel
{
    public partial class Form1 : Form
    {
        ...
        private void btnStart_Click()
        {
          ...
          //  tbPathToFile.Text = "D:\\Google Disk\\МАГИСТРАТРА\\__ДИПЛОМ\\ResultTrue.xls";
           // tbPathToFile.Text = "D:\\Google Disk\\МАГИСТРАТРА\\1(10 семестр)\\ДИПЛОМ\\Для диплома магистра\\ClusterVector\\ClusterVector\\bin\\Debug\\ResultTrue.xls";
        ...
        private void CheckBoxGM_Click(object sender, EventArgs e)
        {
            if (checkBoxGM.Checked)
            {
                ...
                if (rbtnWoman.Checked)
                {
                    OleDbDataAdapter da = new OleDbDataAdapter("Select * from [Ж1$] ", conn);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
                ...
                {
                    OleDbDataAdapter da = new OleDbDataAdapter("Select * from [М1$]", conn);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
                    dataGridViewExcelGlobal.AllowUserToAddRows = false;
                    dataGridViewExcelGlobal.DataSource = dt;
                ...
                for (int k=1;k<dataGridViewExcelGlobal.Columns.Count;k++)
                {
                    ...
                    Console.WriteLine(SummClasters.Count);
                    List<double> SummClastersCopy = new List<double>();
